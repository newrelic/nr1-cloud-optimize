...
exports.calculate = (workloadResults, tags) => {
  const cost = {
    known: 0,
    estimated: 0,
    optimized: 0,
  ...
  workloadResults.forEach(workloadResult => {
    const { guid } = workloadResult;
    const key = guid;
    ...
    (workloadResult?.results || []).forEach(e => {
      if (this.checkTags(e, tags)) {
        if (e.type === 'HOST') {
          if (e.spot) {
...
exports.checkTags = (entity, selectedTags) => {
