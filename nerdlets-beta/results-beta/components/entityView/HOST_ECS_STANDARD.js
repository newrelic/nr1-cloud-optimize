...
import HostEcsContainerModal from './HOSTECSCONTAINERMODAL';

// eslint-disable-next-line no-unused-vars
export default function HostEcsViewStandard(props) {
  ...
  return (
    <>
      <Modal
        isOpen={modalData}
        contentLabel="ECS Container Modal"
        ariaHideApp={false}
      >
        <Button
      ...
      <Card collapsible style={{ marginLeft: '0px' }}>
        <CardHeader
          ...
          title={`${groupTitle || 'HOST - Kubernetes'} (${entities.length})`}
        />
        <CardBody
              ...
              >
                {item.name}
                  ...
                  <EntityTitleTableRowCell
                    ...
                    additionalValue={groupName || item?.clusterName}
                  />
                  ...
                  <TableRowCell>
                    {(item?.EcsContainerData || []).length > 0 && (
                      <Button
                      ...
                      >
                        Containers&nbsp;({item.EcsContainerData.length})
                      </Button>
                    )}
                  </TableRowCell>
                </TableRow>
