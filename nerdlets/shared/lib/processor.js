...
export const pluckCloudInstance = (
  instanceType,
  operatingSystem,
  cloudData
) => {
...
export const processSample = (
  ...
  sample.apps = sample.apmApplicationNames
    ? sample.apmApplicationNames.split('|').filter(Boolean)
    : [];
  ...
  sample.matchedInstance = pluckCloudInstance(
    sample.ec2InstanceType || sample.instanceType,
    sample.operatingSystem,
    cloudData
  );
  if (sample.matchedInstance) {
    sample.instancePrice1 =
      sample.matchedInstance.onDemandPrice * config.discountMultiplier;
...
export const groupInstances = (data, type, val, state, forceGroupBy) => {
  ...
  if (type === 'awsPricingRegion' || type === 'recalc') {
    tempData.forEach((sample, i) => {
      ...
      tempData[i].matchedInstance = pluckCloudInstance(
        tempData[i].ec2InstanceType || tempData[i].instanceType,
        tempData[i].operatingSystem,
        cloudData
      );
      tempData[i].instancePrice1 =
        tempData[i].matchedInstance.onDemandPrice * config.discountMultiplier;
      sample.suggestions = determineCloudInstance(
        config,
        tempData[i],
        cloudData
      );
      ...
      if (tempData[i].suggestions) {
        ...
        tempData[i].instancePrice2 =
          parseFloat(optimizedSuggestion.price) * config.discountMultiplier;
        tempData[i].saving =
          tempData[i].instancePrice1 - tempData[i].instancePrice2;
      } else {
        tempData[i].instancePrice2 = 0;
        tempData[i].saving = 0;
      }
    ...
    .map((value, key) => {
      ...
      value.forEach(instance => {
        if (instance.saving > 0 && instance.instanceType === 'stale') {
          ...
          instance.saving > 0 &&
          (instance.maxCpuPercent < config.optimizeBy ||
            instance.maxMemoryPercent < config.optimizeBy)
        ) {
          summary.optimizedCost += instance.instancePrice2;
          summary.nonOptimizedCost += instance.instancePrice1;
          summary.saving += instance.saving;
      ...
      return { group: key, instances: value, ...summary };
    })
    .value();
...
export const determineCloudInstance = (cfg, instanceData, cloudData) => {
  ...
  // Instance Inclusion
  if (
    instanceData.timeSinceLastReported >
    parseFloat(cfg.lastReportPeriod) * 3600000
  ) {
    return null;
  }
  ...
  if (cfg.staleTransmitBytesPerSecond > 0) {
    traCheck = true;
    if (
      instanceData.transmitBytesPerSecond <= cfg.staleTransmitBytesPerSecond
    ) {
      traStale = true;
    }
  }
