...
import NRLabsFilterBar from '../../../nr-labs-components/NRLabsFilterBar';
...
export default function Results(props) {
  ...
  const options = Object.keys(entityTags).map(tag => {
    return {
      option: tag,
      type: 'string',
      values: entityTags[tag],
      group: tag
    };
  });
          ...
          <Card>
            ...
            >
              <NRLabsFilterBar
                ...
                options={options}
                onChange={d => setTags(d)}
              />
            </CardBody>
          </Card>
        </StackItem>
        <StackItem style={{ width: '8%' }}>
          <SegmentedControl
