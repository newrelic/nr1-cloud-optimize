      ...
      >
        ...
        <Table.Body>
          <Table.Row>
            ...
            <Table.Cell>
              $
              {((data.nonOptimizedCost || data.noc) * 720)
                .toFixed(2)
                .replace(/\d(?=(\d{3})+\.)/g, '$&,')}
            </Table.Cell>
            <Table.Cell>
              $
              {((data.nonOptimizedCost || data.noc) * 720 * 12)
                .toFixed(2)
                .replace(/\d(?=(\d{3})+\.)/g, '$&,')}
            </Table.Cell>
          </Table.Row>
          <Table.Row>
            ...
            <Table.Cell>
              $
              {((data.optimizedCost || data.oc) * 720)
                .toFixed(2)
                .replace(/\d(?=(\d{3})+\.)/g, '$&,')}
            </Table.Cell>
            <Table.Cell>
              $
              {((data.optimizedCost || data.oc) * 720 * 12)
                .toFixed(2)
                .replace(/\d(?=(\d{3})+\.)/g, '$&,')}
            </Table.Cell>
          </Table.Row>
          <Table.Row>
            ...
            <Table.Cell positive>
              $
              {((data.saving || data.s) * 720)
                .toFixed(2)
                .replace(/\d(?=(\d{3})+\.)/g, '$&,')}
            </Table.Cell>
            <Table.Cell positive>
              $
              {((data.saving || data.s) * 720 * 12)
                .toFixed(2)
                .replace(/\d(?=(\d{3})+\.)/g, '$&,')}
            </Table.Cell>
          </Table.Row>
        ...
        <Table.Footer>
          <Table.Row>
            <Table.HeaderCell style={{ textAlign: 'right' }} colSpan={3}>
              {data.nonOptimizedCount || data.noi}/
              {data.totalInstances || data.noi + data.oi} Instances to Optimize
              |{' '}
              {data.nonOptimizedCost || data.noc > 0
                ? (
                    (Math.abs(data.saving || data.s) /
                      (data.nonOptimizedCost || data.noc)) *
                    100
                  ).toFixed(2)
                : 0}
              % Cost Saving <br />
              {data.t ? (
                <span>
                  ...
                  <Button
                    ...
                    onClick={() => this.deleteSnapshot(data.t)}
                  />
                </span>
              ) : (
                <span>
