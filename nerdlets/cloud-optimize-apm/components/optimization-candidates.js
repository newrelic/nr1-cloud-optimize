...
export default class OptimizationCandidates extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    // Potential source of a bug - ensure in parent we aren't changing by reference (i.e. maintain immutability so this works)
  ...
  initializeRecords() {
    const { grouped, userConfig } = this.props;
    const { sort, sortByDefault } = this.state;

    const sortBy = userConfig.sortBy ? userConfig.sortBy : sortByDefault;
    ...
    const data = finalSort && finalSort[0] ? finalSort.instances : [];
