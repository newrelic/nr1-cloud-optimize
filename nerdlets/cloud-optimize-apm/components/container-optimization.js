          ...
          <Grid.Row>
            <Grid.Column>
              <Statistic.Group size="mini" widths="five">
                ...
                <Statistic>
                  <Statistic.Value>
                    {containerMetrics.MaxUniqueDailyContainers}
                  </Statistic.Value>
                  <Statistic.Label>Max Daily Unique Containers</Statistic.Label>
                </Statistic>
                <Statistic>
                  <Statistic.Value>
                    {containerMetrics.MaxCpuSysPerc.toFixed(2).replace(
                      /\d(?=(\d{3})+\.)/g,
                      '$&,'
                    )}
                  </Statistic.Value>
                ...
                <Statistic>
                  <Statistic.Value>
                    {containerMetrics.MaxMemPerc.toFixed(2).replace(
                      /\d(?=(\d{3})+\.)/g,
                      '$&,'
                    )}
                  </Statistic.Value>
                ...
                <Statistic>
                  <Statistic.Value>
                    {(containerMetrics.MaxMemResidentBytes / 1024 / 1024)
                      .toFixed(2)
                      .replace(/\d(?=(\d{3})+\.)/g, '$&,')}
                  </Statistic.Value>
